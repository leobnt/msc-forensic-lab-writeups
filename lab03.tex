...
It is used to store configuration data for system and user applications as well as cached and horistorical data.
You can thus find paging settings along with URLs that have been entered in the Internet Explorer recently in the Registry.


In this report, we will examine a given Windows system and try to uncover the last actions that were taken on the system. \FIXME{WTF?}
...
Fundamentally, the Registry is a big hierachical database of key, value pairs.
Each key has a path and each value has a type.
The types are
    binary,
    string,
    FIXME,
    FOO.
Interestingly, list types are missing \FIXME{dunno whether this is right} and it also lacks so called Schemas, which are basically default values for keys that have either not been set by the user or the application or have been deleted.
The database also knows about access control so that a user cannot necessarily modify the system database but rather her personal database only.\FIXME{Does it have AC?}
This is implemented using file permissions: The users Registry is saved under FIXME per default and the system level database is saved under FIXME. \FIXME{Just guessing. Really. We might need to get rid of that entirely.}

This approach differs from what the GNOME desktop does: A central database called GConf\footnote{It will be obsoleted by DConf for GNOME 3.0 (\href{http://live.gnome.org/TwoPointThirtyone/}{to be released on 2010-09-27}) though} exists but in order to modify the key value pairs, the modifying application has to connect to a central server instance instead of directly modifying files.
Without providing too much detail (simply because it would be out of scope of this report), this has several advantages such as caching, authentification or preserving integrety. \FIXME{Just another (hidden) rant on windows, dunno if it's appropriate. Feel free to get rid of that entirely. Dunno whether it's true anyway}


The Registry can be read using Free Software tools such as FOO or BAR\FIXME{which tools can read it?}.
As the registry provides timestamps of what? Access? Modify? Create? Change? these tools can display them as well.
We found FOO as the best tool to use and an example usage would look like this:
\begin{verbatim}
$ regopen NTUSER.dat
FOO
$
...
In order to get an idea of the order of the actions that were taken on the system, one can sort this information using the timestamp as a key:
\begin{verbatim}
$ regopen | sort > reg-timeline.txt
\end{verbatim}


What does the mtime of a Key mean? not clear.
patching

\subsection{Internet Explorer History}
